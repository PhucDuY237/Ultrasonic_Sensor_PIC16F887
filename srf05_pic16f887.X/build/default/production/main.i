...
{
  PORTDbits.RD0 = RS;
  PORTDbits.RD3 = n & 0x01;
  PORTDbits.RD4 = (n >> 1) & 0x01;
  PORTDbits.RD5 = (n >> 2) & 0x01;
  PORTDbits.RD6 = (n >> 3) & 0x01;
...
__bit get_distance(uint16_t *ticks)
{
  *ticks = 0;
  ...
  while( PORTBbits.RB0 && (*ticks < 23200) )
    *ticks = ( TMR1H << 8 ) | TMR1L;
  ...
  while(1)
  {
    ...
    if (wait_sensor())
    {
      ...
      else
      {
        ...
        LCD_Print(text);
        LCD_Goto(3, 2);
        sprintf(text, "%.2f inches   ", distance_in_inches);
        LCD_Print(text);
        PORTBbits.RB5 = 0;
      }
